...
export interface AuthSecurityRepository {
  ...
  revokeSessions(actorId: string, targetId: string, reason: string): Promise<void>;
...
function authorize(actor: SecurityActor) {
  if (!actor.role?.split(",").includes("platform_operator")) {
    throw new ApplicationError("UNAUTHORIZED", "No tienes permiso para esta operación.");
  }
}
...
export async function findSecurityUser(
  ...
  authorize(actor);
  const parsed = z.email().safeParse(text(email, "El correo", 320).toLowerCase());
  if (!parsed.success) {
...
export async function issueTemporaryPassword(
  ...
  authorize(actor);
  if (!(["WHATSAPP", "IN_PERSON", "OTHER"] as const).includes(input.verificationMethod)) {
