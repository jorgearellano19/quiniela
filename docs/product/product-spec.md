...
### LEAGUE_PLAYOFFS
Regular phase is a configurable partial H2H schedule. Maximum 30 participants. The Admin configures 1 through N-1 regular-phase rounds and 2/4/8/16 qualifiers without exceeding the roster. No groups. After the regular phase, playoffs are played.
...
Starting requires no pending requests, valid phase configuration, the exact prepared DRAFT Round count, and a playable ACTIVE roster: at least 1 for LEAGUE, 2–30 for LEAGUE_PLAYOFFS, and exactly 8, 16, 32, or 64 for GROUP_PLAYOFFS. Starting freezes the roster. After STARTED, the Admin generates the visible persisted league draw or confirms the manual groups before publishing any regular Round. The Admin may complete a Competition only when its required regular Rounds/phases are effectively FINALIZED and its required final winner is resolved: the League winner for LEAGUE, or the Playoff Champion for LEAGUE_PLAYOFFS/GROUP_PLAYOFFS. COMPLETED preserves read-only historical results and locks remaining administrative configuration.
...
## 8. Other scoring
`CLOSEST_VALUE` awards the point to the closest prediction. When `againstRival=true` and H2H applies: exact prediction wins; otherwise the closer prediction wins; if both non-exact predictions have equal distance, neither wins. During a bye, the participant competes against the exact arithmetic mean of other eligible Answers; their own Answer, missing Answers, and restricted participants are excluded. No eligible peer Answers yields zero for a submitted Answer, while unanswered keeps its configured penalty.
...
Each regular H2H Round pairs a Participant with one opponent or an explicit bye. The Participant with the higher Prediction Score for that Round receives 3 H2H Points and the opponent receives 0. If their Prediction Scores tie, each receives 1 H2H Point. A bye awards no H2H Points, although its Prediction Score remains part of phase tiebreakers. For `LEAGUE_PLAYOFFS`, the server securely shuffles the roster once, persists and shows that order, then applies the circle method for the configured number of rounds; retries never redraw. `GROUP_PLAYOFFS` group membership is assigned manually by the Admin, after which the system generates round-robin matchups within each group.

An H2H matchup is `POR_JUGAR` until its first result-complete Question, `PROVISIONAL` while partial results or corrections remain, and `FINAL` when its Round is effectively finished. Qualifiers become official only after every phase Round is effectively FINALIZED and every required tie is resolved.
...
Ranking-based seeding is: Prediction Score DESC, then EXACT_SCORE DESC, then Admin resolves any remaining tie. Initial bracket generation accepts only the derived `OFFICIAL` qualifier order and atomically snapshots it as immutable numbered seeds; qualification decisions cannot be corrected after that snapshot exists. Every stage pairs the highest remaining original seed with the lowest (`1 vs 16`, `2 vs 15`, and so on). Group standings and advancement use the full H2H order in §11 before Admin resolution. `BEST_SEED` means that when a Playoff Matchup remains tied on its H2H Prediction Score, the better original seed advances. `TIEBREAKER_QUESTION` compares the opponents' derived points on the shared Playoff Round tiebreaker Question after their Prediction Scores tie; equal Question points require Admin resolution.
